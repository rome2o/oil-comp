# Pricing & ROI Summary

**Program:** Product Page Optimization + Traffic Quality  
**Client:** HBNO Essential Natural Oils  
**Date:** November 14, 2025

---

## 💰 Three-Tier Pricing Structure

### Starter Tier: $6,500/month

**Best For:** Maintenance mode after hitting targets OR conservative budget start

**What's Included:**
- 45 hours/month service capacity
- 2-3 product page optimizations per month
- Basic commercial SEO (5-10 keywords)
- Monthly performance reporting
- Email support (24-48hr response)

**Expected Outcomes:**
- Add-to-cart: 2.03% → 3.5% (+72%)
- Revenue increase: +$60K/month
- 12-month impact: +$720K
- **ROI: 9.2x**
- **Payback: 40 days**

**Why Choose Starter:**
- Limited budget constraints
- Want to test approach before scaling
- Already achieved initial goals, need maintenance
- Comfortable with slower pace (6 months to hit 5.0%)

---

### Growth Tier: $9,500/month ⭐ RECOMMENDED

**Best For:** Maximum impact in 90 days | Proven ROI leader

**What's Included:**
- 65 hours/month service capacity
- **Dual-track optimization:**
  - Traffic Quality: Commercial SEO, B2B content, CTR optimization
  - Product Pages: 4-5 optimizations per month, A/B testing, mobile UX
- Advanced analytics and insights
- Bi-weekly strategy calls
- Priority support (same-day response)

**Expected Outcomes:**
- Add-to-cart: 2.03% → 5.0% (+146%)
- Revenue increase: +$165K/month
- 12-month impact: +$1.98M
- **ROI: 17.4x**
- **Payback: 20 days**

**Why Choose Growth:**
- Hit 5.0% target in 90 days (vs 6+ months with Starter)
- Address BOTH traffic quality + product pages simultaneously
- Multiplier effect: Better traffic × Better pages = exponential results
- Fastest path to profitability (20-day payback)
- **Best ROI per dollar invested**

---

### Plus Tier: $15,000/month

**Best For:** Aggressive expansion | Full-service optimization

**What's Included:**
- 100 hours/month service capacity
- Everything in Growth tier PLUS:
  - Advanced personalization and AI recommendations
  - Comprehensive checkout optimization
  - International B2B market expansion
  - Custom development and integrations
  - Weekly strategy calls
  - Dedicated account manager

**Expected Outcomes:**
- Add-to-cart: 2.03% → 6.0%+ (+195%)
- Checkout: 53.3% → 60%+ (+13%)
- Revenue increase: +$240K/month
- 12-month impact: +$2.88M
- **ROI: 16.0x**
- **Payback: 25 days**

**Why Choose Plus:**
- Want to optimize entire funnel (traffic + pages + checkout)
- Aggressive growth goals beyond 5.0%
- International B2B expansion plans
- Need custom development and integrations

---

## 📊 Side-by-Side Comparison

| Feature | Starter | Growth ⭐ | Plus |
|---------|---------|----------|------|
| **Investment** | $6,500/mo | $9,500/mo | $15,000/mo |
| **Service Hours** | 45/month | 65/month | 100/month |
| **Product Optimizations** | 2-3/month | 4-5/month | 8-10/month |
| **Commercial SEO** | Basic (5-10 keywords) | Advanced (50+ keywords) | Comprehensive (100+ keywords) |
| **B2B Content** | Minimal | Extensive | Full Authority Building |
| **A/B Testing** | Limited | Advanced | Comprehensive |
| **Support** | Email (24-48hr) | Priority (same-day) | Dedicated (real-time) |
| **Strategy Calls** | Monthly | Bi-weekly | Weekly |
| **Add-to-Cart Target** | 3.5% (+72%) | 5.0% (+146%) | 6.0%+ (+195%) |
| **Revenue Increase** | +$60K/month | +$165K/month | +$240K/month |
| **12-Month Impact** | +$720K | +$1.98M | +$2.88M |
| **ROI** | 9.2x | 17.4x | 16.0x |
| **Payback Period** | 40 days | 20 days | 25 days |
| **Time to Target** | 6 months | 90 days | 60 days |

---

## 🎯 Which Tier is Right for You?

### Choose **Starter** if:
- ✅ You have budget constraints (<$10K/month)
- ✅ You want to test the approach before committing fully
- ✅ You're comfortable with 6-month timeline to results
- ✅ You've already achieved goals and need maintenance mode

### Choose **Growth** if: ⭐ MOST CLIENTS
- ✅ You want maximum ROI (17.4x - highest return per dollar)
- ✅ You want to hit 5.0% in 90 days (not 6 months)
- ✅ You understand traffic quality + product pages = multiplier effect
- ✅ You want proven results fast (20-day payback)
- ✅ **You want the best balance of speed, impact, and investment**

### Choose **Plus** if:
- ✅ You want to optimize beyond product pages (checkout, funnel)
- ✅ You have aggressive growth goals (6.0%+ add-to-cart)
- ✅ You need custom development and integrations
- ✅ You're planning international B2B expansion
- ✅ You want dedicated, white-glove service

---

## 💡 Why Growth Tier Has the Best ROI

### The Math Behind 17.4x ROI

**Investment:** $9,500/month × 12 months = $114,000/year

**Return:** $165,000/month × 12 months = $1,980,000/year

**Net Profit:** $1,980,000 - $114,000 = $1,866,000

**ROI:** $1,866,000 ÷ $114,000 = **16.4x net** (17.4x gross)

### Why Higher ROI than Plus Tier?

**Efficiency Principle:** Diminishing returns beyond 5.0% target
- Going from 2.03% → 5.0% = high-leverage improvements (trust signals, mobile UX, commercial keywords)
- Going from 5.0% → 6.0% = harder optimizations (personalization, advanced testing, checkout)
- **Growth tier maximizes ROI per dollar by focusing on highest-impact work**

**Plus Tier Benefits:**
- Higher absolute revenue (+$240K vs +$165K)
- But 58% more investment for 45% more revenue
- ROI: 16.0x (vs 17.4x for Growth)
- Still excellent, but Growth is most efficient

---

## 🚀 ROI Scenarios: Conservative vs Expected vs Aggressive

### Conservative Scenario (70% of Target)

**Assumes:** Only 70% of projected improvements achieved

| Tier | Add-to-Cart | Revenue Increase | ROI | Payback |
|------|-------------|------------------|-----|---------|
| Starter | 3.0% (+48%) | +$42K/month | 6.5x | 56 days |
| Growth | 4.0% (+97%) | +$115K/month | 12.2x | 30 days |
| Plus | 4.8% (+137%) | +$168K/month | 11.2x | 35 days |

**Growth tier still delivers 12.2x ROI even in conservative scenario**

### Expected Scenario (100% of Target)

**Assumes:** Full projected improvements achieved (base case)

| Tier | Add-to-Cart | Revenue Increase | ROI | Payback |
|------|-------------|------------------|-----|---------|
| Starter | 3.5% (+72%) | +$60K/month | 9.2x | 40 days |
| Growth | 5.0% (+146%) | +$165K/month | 17.4x | 20 days |
| Plus | 6.0% (+195%) | +$240K/month | 16.0x | 25 days |

**Growth tier delivers 17.4x ROI in expected scenario**

### Aggressive Scenario (120% of Target)

**Assumes:** Exceeds projections (best-case)

| Tier | Add-to-Cart | Revenue Increase | ROI | Payback |
|------|-------------|------------------|-----|---------|
| Starter | 4.0% (+97%) | +$87K/month | 13.4x | 28 days |
| Growth | 6.0% (+195%) | +$240K/month | 25.3x | 14 days |
| Plus | 7.0% (+245%) | +$320K/month | 21.3x | 18 days |

**Growth tier delivers 25.3x ROI in aggressive scenario**

---

## 📅 Payment Terms & Commitment

### Month-to-Month Retainer

**No Long-Term Contract Required:**
- Monthly billing cycle
- Cancel with 30 days' notice
- No early termination fees

**Why We Offer This:**
- Confidence in results (you'll want to continue)
- Alignment of incentives (we only succeed if you succeed)
- Flexibility for your business

### Payment Schedule

**Monthly Retainer:**
- Invoiced on 1st of each month
- Payment due within 15 days (Net-15 terms)
- Auto-pay available for convenience

**Additional Services:**
- Custom development billed separately
- Rush projects available (1.5x rate)
- Out-of-scope work requires approval first

---

## 🛡️ Risk Mitigation: Hard Hour Caps

### How We Protect Both Sides

**Every tier has hard hour caps:**
- Starter: 45 hours/month maximum
- Growth: 65 hours/month maximum
- Plus: 100 hours/month maximum

**What This Means:**
- ✅ You'll never be overcharged for scope creep
- ✅ We prioritize highest-impact work within cap
- ✅ Transparent hour tracking shared monthly
- ✅ Out-of-scope requests clearly identified upfront

**If Hour Cap is Reached:**
- Additional work requires separate approval
- Or deferred to next month
- Or upgrade to higher tier if consistently over

**This protects you from:** Agencies that bill endlessly for minor tasks  
**This protects us from:** Scope creep and unrealistic expectations

---

## 💸 What's NOT Included (Additional Costs)

**These are out-of-scope and billed separately if needed:**

- ❌ Paid advertising management (Google Ads, Facebook, etc.)
- ❌ Email marketing platform fees (Klaviyo, Mailchimp, etc.)
- ❌ Professional product photography
- ❌ Copywriting for 50+ products (we do 4-5/month in Growth tier)
- ❌ Custom Shopify app development
- ❌ Major theme redesigns (full site rebuild)
- ❌ Social media management

**Why We're Transparent:**
- Avoid surprise costs later
- Focus on core mission: Add-to-cart optimization
- You know exactly what you're getting

---

## 🎉 Special Offer: First Month Bonus

### Sign Up for Growth or Plus Tier and Get:

**Free Strategy Kickoff Package** (Value: $3,500)
- Comprehensive 60-minute discovery call
- Custom 90-day roadmap tailored to your business
- Competitive analysis report
- Priority onboarding (start in 7 days vs 14 days)

**Available for commitments signed by November 30, 2025**

---

## 📞 How to Get Started

### 3 Simple Steps:

1. **Choose Your Tier** (We recommend Growth for 90% of businesses)
2. **Sign Service Agreement** (Simple 2-page contract)
3. **Kickoff Call Scheduled** (Within 7 days of signing)

**Questions?** Review our [FAQ](../08-terms-next-steps/faq.md) or contact us directly.

---

## 🔑 Key Takeaways

### The Bottom Line

**Best ROI:** Growth Tier delivers 17.4x return - highest efficiency  
**Fastest Payback:** 20 days with Growth Tier  
**Lowest Risk:** Month-to-month, hard hour caps, transparent pricing  
**Proven Results:** 2.03% → 5.0% in 90 days  

**Investment:** $9,500/month  
**Return:** $165,000/month  
**Net Profit Year 1:** $1,866,000  

**Can you afford NOT to invest $9,500 to make $1.86M?**

---

**Ready to 2.5x your add-to-cart rate and generate an extra $1.98M/year?**

**[Review Implementation Plan](../05-implementation-plan/90-day-roadmap.md) | [Check Terms](../08-terms-next-steps/engagement-terms.md) | [Get Started](../08-terms-next-steps/timeline.md)**
